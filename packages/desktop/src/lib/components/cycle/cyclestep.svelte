...
<Flex
	class="justify-between items-center bg-zinc-700 text-white font-semibold {step.state == "ended"
...
>
	...
	<Flex class="self-center items-center group">
		{#if step.type != 'single'}
			<span class="bg-white rounded-full px-2 py-1 text-gray-800 text-xs">
				{step.runCount === undefined ? '?' : step.runCount + 0} / {step.runAmount?.data ||
					'?'}
			</span>
		{/if}
		...
		<div class="rounded-full p-1 bg-white self-center">
			...
			{:else if ["ending", "ended", "skipped"].includes(step.state)}
				<svg
					id="glyphicons-basic"
					xmlns="http://www.w3.org/2000/svg"
					viewBox="0 0 32 32"
					class="{step.state != "ended"
						? 'fill-orange-500'
