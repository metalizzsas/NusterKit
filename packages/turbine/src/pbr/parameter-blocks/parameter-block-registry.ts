...
import type { NumericParameterBlockHydrated, StatusParameterBlockHydrated, StringParameterBlockHydrated } from "$types/hydrated/cycle/blocks/parameter-block-hydrated";
...
export class ParameterBlockRegistry
{
    ...
    {
        // Primitive types
        if (typeof obj === "number") return this.Numeric(obj, ctx);
        if (typeof obj === "string") return this.String(obj, ctx);

        // Status blocks (check first — their keys don't overlap with numeric)
        if ("maintenance_status" in obj) return this.Status(obj as StatusParameterBlocks, ctx);
        if ("product_status" in obj) return this.Status(obj as StatusParameterBlocks, ctx);

        // String blocks
        if ("string" in obj) return this.String(obj as StringParameterBlocks, ctx);

        // Everything else is numeric
        return this.Numeric(obj as NumericParameterBlocks, ctx);
    }
}