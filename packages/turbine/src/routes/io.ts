...
import { ErrorResponseSchema, IOWriteParamsSchema, IOWriteQuerySchema } from "../schemas";

interface IORoutesOpts {
	gates: IOGatesHydrated[];
}

export async function io_routes(fastify: FastifyInstance, opts: IORoutesOpts) {
	const { gates } = opts;
