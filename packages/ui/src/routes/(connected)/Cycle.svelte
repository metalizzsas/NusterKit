<script lang="ts">
	...
	import CycleTimeline from "./CycleTimeline.svelte";
...
{#if cycleData !== undefined && cycleData.status.mode === "created"}
    ...
    <form action="?/startCycle" method="post" use:enhance>
        <Button
            class="group mb-2 gap-2 self-start"
        ...
        >
            {#if ready}
                <Icon src={ArrowRight} class="size-5 transition-transform duration-300 group-hover:translate-x-1" theme="mini"/>
            {/if}
...
{:else if cycleData !== undefined && (cycleData.status.mode === "started" || cycleData.status.mode === "paused")}
    <CycleTimeline {cycleData} />

    <Flex gap={4} class="mt-4">
        ...
        {#if cycleData.status.pausable}
            <form action="?/pauseCycle" method="post" use:enhance>
                ...
                >
                    <Icon src={cycleData.status.mode === "paused" ? Play : Pause} class="size-4" />
                    {$_(`cycle.buttons.${cycleData.status.mode === "paused" ? "resume" : "pause"}`)}
                </Button>
            </form>
        {/if}

        <form action="?/stopCycle" method="post" use:enhance class="ml-auto">
            <Button
                class="group gap-1.5"
    ...
    {#if cycleData.status.overallPausedTime !== undefined && cycleData.status.overallPausedTime > 0}
        <p class="mt-3 text-sm text-zinc-600 dark:text-zinc-400">
            ...
            <span class="font-semibold">{$_('cycle.eta.paused')}</span>
            <span class="font-mono tabular-nums">{parseDurationToString(cycleData.status.overallPausedTime)}</span>
        </p>
    {/if}
    ...
    <Flex gap={2} direction={"col"}>
        {#each cycleData.steps.filter(s => s.isEnabled) as step, index (step.name)}
            <CycleStep {step} stepNumber={index + 1} />
        {/each}
    </Flex>
